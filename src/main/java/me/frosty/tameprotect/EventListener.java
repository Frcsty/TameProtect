...
public class EventListener implements Listener
{
    ...
    private Set<UUID>   tameOut = new HashSet<>();

    public EventListener(TameProtect plugin)
    {
        this.plugin = plugin;
    }
    ...
    public void onTameAnimal(EntityTameEvent event)
    {
        ...
        if (event.getOwner() instanceof Player)
        {
            ...
            if (owner.hasPermission("tameprotect.protect"))
            {
                ...
                if (tameOut.contains(owner.getUniqueId()))
                {
                    return;
                }
    ...
    public void onMountAnimal(VehicleEnterEvent event)
    {
        if (event.getEntered() instanceof Player && event.getVehicle() instanceof Tameable)
        {
            ...
            if (player.hasPermission("tameprotect.override"))
            {
                return;
            }
    ...
    public void onEnvDamage(EntityDamageEvent event)
    {
        ...
        if (plugin.getConfig().getBoolean("environmental_damage"))
        {
            return;
        }
        ...
        if (plugin.getConfig().getBoolean("damage_while_riding") && riding.size() > 0)
        {
            return;
        }
